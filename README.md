...
## 언어별 Youtube 구독자 수 표현하기

Youtube에서 구독자 정보를 보면 일정 수 단위로 끊어서 표시합니다. 예컨대 만 명이면 10000이 아닌, 1만 명 이렇게 표현하는 것입니다. 최근 스쿱된 플레이리스트의 유튜브 영상 정보와 댓글 정보를 보여주는 기능에서 좋아요 및 구독자 수를 보여주는 기능이 추가되었고 이를 대응했습니다.

이 기능을 제공하기 위해 Youtube를 분석하니 고려할 사항이 크게 3가지로 나뉘었습니다.

1. 언어별로 다른 사람을 표현하는 문법이 다름 대응
2. 나라별로 사용 및 요약시키는 수 단위가 다른 점 대응
3. 수를 요약시키는 방법 및 정책 정의

1번, 2번 고려사항은 나라별로 다른 case로 나누어 처리했고, 3번은 Int에 소수점 표시로 변환하는 메서드를 추가하여 대응했습니다.
